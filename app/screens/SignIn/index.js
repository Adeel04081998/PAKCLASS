...
export default function SignIn({ navigation }) {
  ...
  // const [email, setEmail] = useState(__DEV__ ? 'Ad@gmail.com' : '');
  const [email, setEmail] = useState(__DEV__ ? 'Aliahmad@gmail.com' : '');


  const [password, setPassword] = useState(__DEV__ ? 'abc12345' : '');
  ...
  const onLogin = () => {
    if (email == '' || password == '') {
      ...
      postRequest(Endpoints.Login, {
        ...
        if (status === 'Success') {
          dispatch(setUserAction({ user: res?.data?.user, access_token: res?.data?.access_token }))
